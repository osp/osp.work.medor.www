...
def retail_outlet_as_csv(request):
    ...
    for r in RetailOutlet.objects.filter(is_published=True, is_article_27=False):
        writer.writerow([
            r.name,
...
def article_27_as_csv(request):
    ...
    for r in RetailOutlet.objects.filter(is_published=True, is_article_27=True):
        writer.writerow([
            r.name,
            "%s, %s %s, %s" % (r.address, r.zip_code, r.city, r.country),
            "%s" % r.latitude,
